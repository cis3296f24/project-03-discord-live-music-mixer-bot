...
class vc(commands.Cog):
    def __init__(self, bot):
        try:
            discord.opus.load_opus('/opt/homebrew/lib/libopus.dylib')  # Adjust this path if necessary
            print("Opus library loaded successfully.")
        except Exception as e:
            print(f"Failed to load Opus library: {str(e)}")
        self.audiopath = os.path.join("/opt/homebrew/bin/ffmpeg")
        self.test_ffmpeg_path()
        self.test_ffmpeg_execution()
        self.test_ffmpeg_in_path()
    ...
    def test_ffmpeg_path(self):
        if os.path.exists(self.audiopath):
            ...
            print("FFmpeg executable not found at the specified path")

    def test_ffmpeg_execution(self):
        try:
            subprocess.run([self.audiopath, "-version"], check=True, capture_output=True)
    ...
    def test_ffmpeg_in_path(self):
        ...
        if ffmpeg_path:
            print(f"FFmpeg found in system PATH at: {ffmpeg_path}")
        else:
